...
## 2026-05-16: カード選択時に重なった下のカードが選ばれるバグ修正

**症状**: SELECTING_TARGET 状態で手前のカードをタップすると、下に重なったカードが選択されることがある。

**原因**: `Card.gd` がグローバル `_input()` フックでクリック判定を行っており、イベント配信順がシーンツリーの `add_child` 順（初期生成順）に固定され、z_index（視覚的前後）と一致しなかった。

**修正**: `_input()` + `_is_point_inside()` を撤去し、Area2D の `_input_event()` に置き換え。Godot が z_index を尊重して手前の Area2D にのみイベントを届けるため、正しいカードが選択される。

**修正ファイル**: `scripts/Card.gd`
**テスト**: 全69件通過

## 2026-05-16: カード選択がまったく反応しないバグ修正（前回修正の回帰）

**症状**: 「能力を使う」ボタン押下後のカード選択状態でカードをクリックしても反応しない。

**原因**: 前回の修正（`_input()` → `_input_event()`）が回帰を引き起こした。
`_input_event()` は Area2D フィジックスピッキングを使うが、上に被さる `GameUI`（Control, mouse_filter=STOP）がマウスイベントを先に消費するため、カードまでイベントが届かなかった。

**修正**: `_input()` に戻す（GameUI の mouse_filter に関係なく全ノードへ配信される）。
加えて、z_index を使った重なりチェック `_is_occluded_by_front_card()` を追加し、自分より手前のカードが同座標を覆っている場合は発火しないようにした（前回の z_index 並び順バグも同時に解決）。
