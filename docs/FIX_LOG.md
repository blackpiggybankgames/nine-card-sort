...
| FIX-024 | ABILITY | カード4,5の能力見直し | 4:隣接スワップ、5:中央引き出しに変更 | 2026-03-09 |
| FIX-025 | ABILITY | カード4を難易度アップ | 4:隣接スワップ→2番目落としに変更 | 2026-03-09 |
| FIX-026 | BUG | 能力3で一番端のカードが選択できる | card_index > 0 → card_index > 1 に修正 | 2026-03-15 |
...
### FIX-026: 能力3で一番端のカードが選択できる

**報告**: docs/temp/fix-bug.md より
**優先度**: !!! 致命的
**原因**: `set_non_edge_selectable()`で`card_index > 0`としていたが、`deck_data`には発動カードを含む9枚が入っている。そのため元のindex 1（残り8枚のindex 0 = 左端）が選択可能になっていた。

**修正**:
- `card_index > 0` → `card_index > 1` に変更
- これにより発動カード除外後の8枚で、両端（index 0, 7）が正しく選択不可になる

**変更ファイル**:
- `scripts/DeckDisplay.gd`
  - `set_non_edge_selectable()`の条件を修正
  - コメントを追加して配列構造を明確化
